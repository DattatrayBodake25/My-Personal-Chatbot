...
pinecone.init(api_key='my_pinecone_api_key', environment='my_env_name')
...
def query_refiner(conversation, query):
    ...
    # Check if 'choices' and 'text' keys are present
    if 'choices' in response and response['choices'] and 'message' in response['choices'][0]:
        ...
        st.warning("Failed to extract refined query. Check OpenAI response structure.")
        return ""
